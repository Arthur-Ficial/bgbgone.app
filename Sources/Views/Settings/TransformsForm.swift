...
struct TransformsForm: View {
    ...
    var body: some View {
        ClickRowDisclosure(title: "Foreground transforms", isExpanded: $isExpanded) {
    ...
    private var scaleRow: some View {
        let isOn = Binding(
            get: { config.fgScale != nil },
            set: { config.fgScale = $0 ? (config.fgScale ?? 1.0) : nil }
        )
        VStack(alignment: .leading, spacing: 4) {
            Toggle("Scale", isOn: isOn)
            if let v = config.fgScale {
                HStack {
                    Slider(value: Binding(
                        get: { v },
                        set: { config.fgScale = $0 }
                    ), in: 0.1...3.0)
    ...
    private var translateRow: some View {
        let isOn = Binding(
            get: { config.fgTranslateX != nil },
            set: { on in
                config.fgTranslateX = on ? (config.fgTranslateX ?? 0) : nil
                config.fgTranslateY = on ? (config.fgTranslateY ?? 0) : nil
            }
        )
        Toggle("Translate", isOn: isOn)
        if let x = config.fgTranslateX, let y = config.fgTranslateY {
            HStack {
                Stepper("X: \(x) px", value: Binding(
                    get: { x },
                    set: { config.fgTranslateX = $0 }
                ), in: -2000...2000)
                Stepper("Y: \(y) px", value: Binding(
                    get: { y },
                    set: { config.fgTranslateY = $0 }
                ), in: -2000...2000)
            }
        }
    }
    ...
    private var rotateRow: some View {
        let isOn = Binding(
            get: { config.fgRotate != nil },
            set: { config.fgRotate = $0 ? (config.fgRotate ?? 0) : nil }
        )
        VStack(alignment: .leading, spacing: 4) {
            Toggle("Rotate", isOn: isOn)
            if let v = config.fgRotate {
                HStack {
                    Slider(value: Binding(
                        get: { v },
                        set: { config.fgRotate = $0 }
                    ), in: -180...180)
    ...
    private var flipRow: some View {
        let isOn = Binding(
            get: { config.fgFlip != nil },
            set: { config.fgFlip = $0 ? (config.fgFlip ?? "horizontal") : nil }
        )
        Toggle("Flip", isOn: isOn)
        if let v = config.fgFlip {
